...
注意：暂不支持直接嵌套 iframe 组件，在 myDrag 组件中嵌套 iframe 会导致鼠标事件无法正确判断，这是因为 iframe 创建了一个独立的上下文，当鼠标进入 iframe 时，父文档的鼠标事件会丢失。
可以自行增加 iframe 嵌套的处理方法，或者将鼠标移入当前组件事件去掉，这样就可以嵌套 iframe 了。
